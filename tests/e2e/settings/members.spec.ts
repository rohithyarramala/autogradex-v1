...
const test = base.extend<MemberFixture>({
  ...
  memberPage: async ({ page }, use) => {
    const apiKeysPage = new MemberPage(page, organization.slug);
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(apiKeysPage);
  },
  secondUserJoinPage: async ({ page }, use) => {
    const joinPage = new JoinPage(page, secondUser, secondUser.organization.name);
    await joinPage.goto();
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(joinPage);
  },
...
test('Should be able to get the list of members', async ({
  ...
  await memberPage.organizationMemberExists(user.name, user.email, 'OWNER');
});
...
test('New member should be able to accept the invitation', async ({
  ...
  await loginPage.gotoInviteLink(invitationLink, organization.name);
  ...
  await loginPage.invitationAcceptPromptVisible(organization.name);
...
test('Existing user should be able to accept the invitation', async ({
  ...
  await loginPage1.gotoInviteLink(invitationLink, organization.name);
  ...
  await loginPage1.invitationAcceptPromptVisible(organization.name);
...
test('Should be able to create invite using domain', async ({
  ...
  const loginPage1 = new LoginPage(page1);
  await loginPage1.gotoInviteLink(domainInviteLink, organization.name);
...
test('Should not allow to invite a member with invalid domain', async ({
  ...
  await loginPage.gotoInviteLink(domainInviteLink, organization.name);
